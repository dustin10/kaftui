...
impl SchemasState {
    ...
    fn select_first_subject(&mut self) {
        ...
        self.subjects_scroll_state = self.subjects_scroll_state.position(0);
    ...
    fn select_next_subject(&mut self) {
        ...
        self.subjects_scroll_state = self.subjects_scroll_state.position(idx);
    ...
    fn select_next_schema_version(&mut self) -> Option<i32> {
        ...
        self.versions_scroll_state = self.versions_scroll_state.position(idx);
...
impl Schemas {
    ...
    fn render_references(&self, frame: &mut Frame, area: Rect) {
    ...
    fn select_first_schema_version(&mut self) {
        if let Some(version) = self.state.select_first_schema_version() {
            self.load_selected_subject_version(version);
        }
    }
    ...
    fn load_selected_subject_latest(&mut self) {
        if let Some(selected) = self.state.selected_subject.as_ref() {
            ...
            let registered_schema = self.load_subject_version(selected, None);

            let versions = futures::executor::block_on(async {
                self.client
                    .get_all_versions(selected)
                    .await
    ...
    fn load_selected_subject_version(&mut self, version: i32) {
        if let Some(selected) = self.state.selected_subject.as_ref() {
            tracing::info!(
            ...
            let registered_schema = self.load_subject_version(selected, Some(version));
        ...
        match event.code {
            KeyCode::Char(c) => match c {
                ...
                _ => match self.state.active_widget {
                    ...
                    // TODO: implement scrolling for references
